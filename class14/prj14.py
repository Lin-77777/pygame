...
class Player:
    ...
    def take_damage(self, invincible_duration=60):
        """
        當太空船未處於無敵狀態時，啟動無敵狀態，持續 invincible_duration 幀
        可於此方法加入受傷音效或特效
        """
...
##################### 敵機類別 #####################
class Enemy:
    def __init__(self, x, y, width, height, speed, burner_img=None, img_explode=None):
        """
        初始化敵機物件\n
        x, y: 敵機左上角座標\n
        width, height: 敵機寬高\n
        ...
        if enemy_images:
            ...
            self.img = None
        # 火焰推進效果相關屬性
        self.burner_img = burner_img  # 火焰圖片
        self.burn_shift = 0  # 火焰動畫上下晃動位移
        # 爆炸動畫相關屬性
        self.EXP = False  # 是否正在爆炸動畫中
        self.exp_count = 0  # 爆炸動畫目前幀數
        self.exp_max = 30  # 爆炸動畫總幀數(6張圖*5幀=30)
        self.img_explode = img_explode if img_explode else []  # 爆炸動畫圖片列表
    ...
    def reset(self, bg_x):
        ...
        if enemy_images:
            self.img = random.choice(enemy_images)
        self.EXP = False  # 重生時結束爆炸狀態
        self.exp_count = 0

    def explode(self):
        """
        觸發爆炸動畫，將EXP設為True並重設計數器
        """
        self.EXP = True
        self.exp_count = 0
    ...
    def draw_explode(self, screen):
        """
        繪製爆炸動畫，依exp_count顯示對應爆炸圖片，動畫結束後自動reset重生
        """
        if self.img_explode:
            idx = self.exp_count // 5  # 每5幀換一張圖
            if idx < len(self.img_explode):
                img = pygame.transform.scale(
                    self.img_explode[idx], (self.width, self.height)
                )
                screen.blit(img, self.rect)
                self.exp_count += 1
            else:
    ...
    def draw(self, screen):
        ...
        if self.burner_img:
            # 火焰圖片需旋轉180度(朝下)
            burner_rotated = pygame.transform.rotate(self.burner_img, 180)
            # 火焰寬度設為敵機寬度的1/4，高度等比例縮放
            burner_w = self.width // 4
            burner_h = int(
                burner_rotated.get_height() * (burner_w / burner_rotated.get_width())
            )
            # 火焰動畫上下晃動(循環0~11)
            self.burn_shift = (self.burn_shift + 2) % 12
            # 火焰中心對齊敵機底部中央，並有微小上下位移
            burner_x = self.rect.centerx - burner_w // 2
            burner_y = self.rect.top - burner_h // 2 + self.burn_shift
            burner_img_scaled = pygame.transform.scale(
                burner_rotated, (burner_w, burner_h)
            )
            screen.blit(burner_img_scaled, (burner_x, burner_y))
        # 再繪製敵機本體
        if self.img:
...
class CollisionManager:
    def __init__(
        ...
        missile_list: 飛彈物件列表
        emy_list: 敵機物件列表
        score_manager: 分數管理器(可選)
        audio_manager: 音效管理器(可選)
        player: 玩家物件(可選)
        """
        self.missile_list = missile_list
        self.emy_list = emy_list
        self.score_manager = score_manager
        self.audio_manager = audio_manager
        self.player = player  # 新增：記錄玩家物件

    def is_hit(self, obj1, obj2, r=None):
        """
        判斷兩個物件是否碰撞(以中心點距離為準)
        obj1, obj2: 需有 rect 屬性的物件
        r: 碰撞半徑(若未指定則取敵機寬度的一半)
        return: True/False
        """
        x1, y1 = obj1.rect.center
        x2, y2 = obj2.rect.center
        if r is None:
            r = obj2.rect.width // 2  # 預設以敵機寬度一半為碰撞半徑
        return (x1 - x2) ** 2 + (y1 - y2) ** 2 < r**2

    def check_collision(self):
        """
        檢查所有飛彈與敵機的碰撞，以及玩家與敵機的碰撞
        - 若飛彈碰撞: 飛彈失效, 敵機觸發爆炸動畫, 分數+10, 播放音效
        - 若玩家碰撞敵機: 若玩家未無敵，觸發玩家無敵與敵機爆炸
        """
        # 處理飛彈與敵機碰撞
        for msl in self.missile_list:
            if msl.active:
                for emy in self.emy_list:
                    if self.is_hit(msl, emy, r=40):
                        msl.active = False  # 飛彈失效
                        emy.explode()  # 觸發敵機爆炸動畫，不立即重生
                        # 擊中敵機時加分與播放音效
        ...
        if self.player:
            for emy in self.emy_list:
                # 只在敵機未爆炸時檢查
                if not emy.EXP and self.is_hit(self.player, emy, r=40):
                    if not self.player.invincible:
                        self.player.take_damage(invincible_duration=60)  # 啟動無敵
                        emy.explode()  # 敵機爆炸
                        # 這裡可加入受傷音效
...
class ScoreManager:
    def __init__(self):
        ...
        self.score = 0
        # 使用微軟正黑體，字型大小32
        self.font = pygame.font.Font("C:/Windows/Fonts/msjh.ttc", 32)
    ...
    def draw(self, screen):
        ...
        在畫面左上角顯示分數
        """
        score_text = self.font.render(f"分數: {self.score}", True, (255, 255, 0))
...
class AudioManager:
    def __init__(self):
        ...
        """
        try:
            self.hit_sound = pygame.mixer.Sound(os.path.join("image", "hit.mp3"))
...
# 載入爆炸動畫圖片（共6張，依序為explode0.png~explode5.png）
# 將爆炸動畫圖片存入 img_explode 列表
img_explode = []
for i in range(1, 6):
    img = pygame.image.load(os.path.join("image", f"explosion{i}.png"))
...
missile_speed = -25  # 飛彈速度(負值向上)
# #################### 飛彈連發設定 ####################
# 設定飛彈最大同時存在數量
MISSILE_MAX = 15  # 最多同時存在10發飛彈
# 建立飛彈物件列表
missiles = [
...
for i in range(emy_num):
    ...
    # 傳入火焰推進圖片 img_burner 及爆炸動畫圖片 img_explode
    enemy = Enemy(
        enemy_x, enemy_y, enemy_w, enemy_h, enemy_speed, img_burner, img_explode
    )
    emy_list.append(enemy)

# #################### 分數與音效管理器初始化 ####################
score_manager = ScoreManager()  # 分數管理器
audio_manager = AudioManager()  # 音效管理器

# #################### 主程式 ####################
# 初始化碰撞管理器，傳入飛彈與敵機列表、分數與音效管理器、玩家物件
collision_manager = CollisionManager(
    missiles, emy_list, score_manager, audio_manager, player
)
