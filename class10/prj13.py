...
class Player:
    ...
    def draw(self, display_area):
        ...
        if self.flying and self.sprites and "propeller" in self.sprites:
            ...
            propeller_width = int(self.rect.width * 1.2)  # 飛行帽寬度比主角寬20%
            propeller_height = int(propeller_width * 0.647)  # 保持原始比例
            ...
            propeller_rect.bottom = self.rect.top
...
try:
    ...
    print(f"載入音效成功: {jump_sound}")
except Exception as e:
    print(f"載入音效時發生錯誤: {e}")
    jump_sound = None  # 如果載入失敗，設為 None
# 載入彈簧音效
try:
    spring_sound = pygame.mixer.Sound("sound/spring.mp3")  # 載入彈簧音效
    print(f"載入彈簧音效成功: {spring_sound}")
except Exception as e:
    print(f"載入彈簧音效時發生錯誤: {e}")
    spring_sound = None  # 如果載入失敗，設為 None
try:
    # 載入帽子音效
    propeller_sound = pygame.mixer.Sound("sound/propeller.mp3")  # 載入飛行帽音效
    print(f"載入音效成功: {propeller_sound}")
except Exception as e:
    print(f"載入飛行帽音效時發生錯誤: {e}")
    propeller_sound = None
try:
    broke_sound = pygame.mixer.Sound("sound/broke.mp3")  # 載入破壞音效
    print(f"載入音效成功: {broke_sound}")
except Exception as e:
    print(f"載入破壞音效時發生錯誤: {e}")
    broke_sound = None  # 如果載入失敗，設為 None
######################遊戲視窗設定######################
bg_x = 400  # 設定視窗寬度
bg_y = 600  # 設定視窗高度
...
while True:
    ...
    if game_over:
        game_over_text = font.render(
            "遊戲結束！按任意鍵重新開始", True, (0, 0, 0)  # 使用黑色文字
        )
        highest_score_text = font.render(
            f"最高分數: {highest_score}", True, (0, 0, 0)  # 使用黑色文字
        )
